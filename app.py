...
# It will config the name for a page
st.set_page_config(
...
with col1:
    ...
    resume_file = st.file_uploader("Upload resume (.txt, .pdf, .docx)", type=["txt", "pdf", "docx"], key="resume") #file uploader
    resume_text_input = st.text_area("Or paste resume text", height=220, key="resume_text") #text area
...
if st.button("🚀 Build Career Coach RAG Index", type="primary"):
    if not resume_text.strip() or not jd_text.strip():
        st.error("Please upload or paste both Resume and Job Description.") #error message to the user
    else:
        with st.spinner("Running RAG stages: loading → chunking → embeddings → vector database..."):  #loads and running the stages 
            docs = create_documents(resume_text, jd_text) # this resume text and JD text converts into Langchain documents format
            chunks = split_documents(docs, chunk_size=chunk_size, chunk_overlap=chunk_overlap) #this docs uploaded to split_documents to parse the data one by one
...
if st.session_state.vectorstore:
    ...
    if st.button("🤖 Get Career Coach Answer"): #run_career_coach and llm et activated and from the vectorDB the relevent r=info gets retrieved
